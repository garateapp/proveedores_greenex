...
interface Props {
    initialContratista: ContratistaOption | null;
    initialPeriodo: {
        periodo_ano: number;
...
interface SearchContratistasResponse {
    data: ContratistaOption[];
}
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Centro de Carga Contratistas',
...
export default function CentroCarga({
    ...
    const [periodoMes, setPeriodoMes] = useState(
        initialPeriodo.periodo_mes !== null
            ? String(initialPeriodo.periodo_mes)
            : '',
    );
    const [search, setSearch] = useState(
        initialContratista
            ? `${initialContratista.rut} · ${initialContratista.nombre_mostrado}`
            : '',
    );
    const [searchResults, setSearchResults] = useState<ContratistaOption[]>([]);
    const [searching, setSearching] = useState(false);
    const [selectedContratista, setSelectedContratista] =
        useState<ContratistaOption | null>(initialContratista);
    ...
    const selectedContratistaId = selectedContratista?.id ?? '';
    ...
    useEffect(() => {
        ...
        if (
            selectedContratista &&
            trimmedSearch ===
                `${selectedContratista.rut} · ${selectedContratista.nombre_mostrado}`
        ) {
            setSearchResults([]);
            return;
        }
        ...
        const timeout = setTimeout(async () => {
            ...
            try {
                const response = await fetch(
                    `/centro-carga-contratistas/contratistas?search=${encodeURIComponent(trimmedSearch)}`,
                    {
                        headers: {
                ...
                const payload =
                    (await response.json()) as SearchContratistasResponse;
    ...
    const loadRequirements = useCallback(
        async (
            ...
            nextPeriodoAno: string,
            nextPeriodoMes: string,
        ): Promise<void> => {
            if (!nextPeriodoAno) {
                setRequirements({
                    tipos_documentos: [],
            ...
            try {
                ...
                params.set('periodo_ano', nextPeriodoAno);
                if (nextPeriodoMes) {
                    params.set('periodo_mes', nextPeriodoMes);
                }

                const response = await fetch(
                    `/centro-carga-contratistas/contratistas/${contratistaId}/requerimientos?${params.toString()}`,
                    {
                        headers: {
                            Accept: 'application/json',
                        },
    ...
    useEffect(() => {
        ...
        return () => clearTimeout(timeout);
    }, [loadRequirements, periodoAno, periodoMes, selectedContratista]);

    const selectContratista = useCallback(
        async (contratista: ContratistaOption): Promise<void> => {
            setSelectedContratista(contratista);
            setSearch(`${contratista.rut} · ${contratista.nombre_mostrado}`);
            setSearchResults([]);
            clearItems();
        ...
        [clearItems, loadRequirements, periodoAno, periodoMes],
    );
    ...
    const addFiles = useCallback(
        (incomingFiles: File[]) => {
            if (!selectedContratistaId) {
                setDropzoneError(
                    'Selecciona un contratista antes de cargar archivos.',
                );
                return;
            }
        ...
        [enqueueOCR, selectedContratistaId],
    );
    ...
    const uploadSingle = useCallback(
        async (item: CargaArchivoItem): Promise<void> => {
            const year = periodoAno.trim();
            const month = periodoMes.trim();
            ...
            await new Promise<void>((resolve) => {
                ...
                formData.append(
                    'contratista_id',
                    selectedContratista.id.toString(),
                );
                formData.append(
                    'tipo_documento_id',
                    tipoDocumentoId.toString(),
                );
                formData.append('periodo_ano', year);
                if (month) {
                    formData.append('periodo_mes', month);
                }
                ...
                xhr.open('POST', '/centro-carga-contratistas/documentos', true);
        ...
        [periodoAno, periodoMes, selectedContratista, updateItem],
    );
            ...
            <div className="space-y-6">
                    ...
                    <CardContent className="space-y-3">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            <div className="space-y-1">
                                ...
                                <Input
                                    ...
                                    onChange={(event) =>
                                        setPeriodoAno(event.target.value)
                                    }
                                />
                            </div>
                            <div className="space-y-1">
                                <Label htmlFor="periodo_mes">
                                    Mes (opcional)
                                </Label>
                                <Input
                                    id="periodo_mes"
                                    ...
                                    placeholder="1-12"
                                    onChange={(event) =>
                                        setPeriodoMes(event.target.value)
                                    }
                                />
                            </div>
                        </div>
                        ...
                        {searchResults.length > 0 && (
                            <div className="max-h-56 space-y-2 overflow-auto rounded-lg border border-border/70 bg-white/85 p-2">
                                {searchResults.map((contratista) => (
                                    <button
                                        key={contratista.id}
                                        type="button"
                                        onClick={() =>
                                            void selectContratista(contratista)
                                        }
                        ...
                        {selectedContratista && (
                            <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--brand-green)]/30 bg-[var(--brand-lime)]/10 px-3 py-2 text-sm">
                                ...
                                <span className="font-semibold">
                                    {selectedContratista.nombre_mostrado}
                                </span>
                                <span className="text-muted-foreground">
                                    {selectedContratista.rut}
                                </span>
                            </div>
                        )}
                    </CardContent>
                ...
                {!selectedContratista && (
                    <div className="flex items-center gap-2 rounded-xl border border-border/70 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
                        <AlertCircle className="size-4" />
                        Selecciona un contratista para habilitar el Centro de
                        Carga.
                    </div>
                )}

                {selectedContratista && !periodoAno.trim() && (
                    <div className="flex items-center gap-2 rounded-xl border border-[var(--brand-orange)]/35 bg-[var(--brand-orange)]/10 px-4 py-3 text-sm text-[var(--brand-orange-strong)]">
                        <AlertCircle className="size-4" />
                        Ingresa el año del período para cargar documentos.
                    </div>
                )}
            </div>
        </>
